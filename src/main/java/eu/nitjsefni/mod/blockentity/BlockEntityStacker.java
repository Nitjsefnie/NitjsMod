...
public class BlockEntityStacker extends BlockEntity implements NamedScreenHandlerFactory, ImplementedInventory {
    ...
    private final DefaultedList<ItemStack> inventory = DefaultedList.ofSize(ScreenHandlerBlockStacker.SLOT_SIZE, ItemStack.EMPTY);

    public BlockEntityStacker(BlockPos blockPos, BlockState blockState) {
        super(NitjsBlockEntities.STACKER_BLOCK_ENTITY, blockPos, blockState);
    }
        ...
        for (int sourceSlot = 0; sourceSlot < source.size(); sourceSlot++) {
            ...
            if (!sourceStack.isEmpty() &&
                ...
                if (destSlot != null) {
                    ...
                    int full = destStack.getCount();
                    int free = destStack.getMaxCount() - full;
                    int count = sourceStack.getCount();
                    int increase = Math.min(count, free);
                    if (increase > 0) {
                        sourceStack.decrement(increase);
                        if (sourceStack.getCount() < 1) {
                            source.removeStack(sourceSlot);
                        }
                        destStack.increment(increase);
                        return;
                    }
                }
            }
